...
function Login()
	...
	-- Информация слева
		...
		Bar.MidL(WIGHT,20, "&6Перезапуск компа -")
	...
	if p== Bar.PASSES[1] or p == Bar.PASSES[2] or p == Bar.PASSES[3] then
		if nick == Bar.NICK[1] or nick == Bar.NICK[2] or nick == Bar.NICK[3] then
			login = true
