...
function Rules(nick)
	if (login) then
		...
		Bar.MidL(WIGHT,6, "Барашик 100% не гей")
		Bar.MidL(WIGHT,7, "У него писюн 15 см")
		Bar.MidL(WIGHT,8, "Ваня пидорас конченный")
...
function DrawProgs(s)
	...
	if sel ~= 0 then
		if (fs.exists(shell.getWorkingDirectory() .. "/" .. progs[sel][1][1] .. ".lua")) then
			Bar.Button(mid-21,33,20,3,COLOR1,COLOR2,"Открыть")
			Bar.Button(mid+1,33,20,3,COLOR1,COLOR2," Обновить")
		else
			Bar.Button(mid-10,33,20,3,COLOR1,COLOR2,"Скачац")
		end
		g.setForeground(COLOR1)
		Bar.MidR(WIGHT,28,"Необходимые компоненты:")
		local comp = ""
	...
	if sel ~= 0 then
		if (fs.exists(shell.getWorkingDirectory() .. "/" .. progs[sel][1][1] .. ".lua")) then
			if w>=mid-21 and w<=mid-2 and h>=33 and h<=35 then
				...
				g.setForeground(COLOR1)
				print("\nЕсли ты это видишь, то ты рукожоп.")
				print("\nКликай на монитор шобы перезагрузица")
