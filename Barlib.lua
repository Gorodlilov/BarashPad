...
function Bar.Read(settings)
    ...
    while true do
		if settings.center then
			SX = sx - unicode.len(settings.text) / 2
		end
        local event, address, char, code, nick = event.pull(settings.blink and math.min(0, nextBlink - computer.uptime())) --, "key_down"
        if char == 13 and event == "key_down" then
            local char, fg, bg = g.get(SX + unicode.len(settings.text), sy)
			if settings.center then
				SX = sx - unicode.len(settings.text) / 2
			end
            g.set(SX + unicode.len(settings.text), sy, char or " ")
            if settings.nick then
				return settings.text, nick
			end
			return settings.text
        elseif char == 8 and event == "key_down" then
            settings.text = unicode.sub(settings.text, 1, -2)
			if settings.center then
				SX = sx - unicode.len(settings.text) / 2
			end
            g.set(SX, sy, (not settings.mask and settings.text or settings.mask:rep(unicode.len(settings.text))) .. "  ")
			if settings.center then
				g.set(SX-1, sy, " ")
			end
        elseif char and char ~= 0 and event == "key_down" then
        ...
        elseif event == "clipboard" and settings.clip then
